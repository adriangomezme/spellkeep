-- Align the bulk-import RPC's "variant" counts with the rest of the UI.
-- Previous versions returned ROW count as variants (one per print ×
-- condition × language). The hub + binder headers use a different
-- definition: print × finish (normal/foil/etched). For Secret Lair
-- that's 1,608 variants vs. 1,422 rows — the user rightly wants a
-- single consistent number.
--
-- Compute `variants` per row as: # of finishes with qty > 0 after the
-- upsert. A row with normal=4 + foil=1 contributes 2 variants.
...
begin
  ...
  upserted as (
    ...
    returning
      ...
      -- Finish-level variant count for this row AFTER the upsert.
      (case when quantity_normal > 0 then 1 else 0 end)
      + (case when quantity_foil   > 0 then 1 else 0 end)
      + (case when quantity_etched > 0 then 1 else 0 end) as variant_count
  ),
  joined as (
    select
      u.was_inserted,
  ...
  select
    ...
    sum(case when was_inserted then variant_count else 0 end)::int,
    sum(case when was_inserted then 0 else variant_count end)::int
