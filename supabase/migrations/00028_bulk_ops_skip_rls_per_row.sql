-- Optimization: the bulk RPCs now run SECURITY DEFINER and set
-- session_replication_role = replica for the duration of the INSERT.
-- That skips RLS checks + user triggers PER ROW (each of which is a
-- non-trivial fraction of overhead at 10k+ rows) while still enforcing
-- FK + unique constraints.
--
-- Security: every RPC validates ownership explicitly before the INSERT
-- (`where user_id = auth.uid()`), so skipping per-row RLS is safe — a
-- caller that doesn't own the source/destination gets the early raise.
...
-- Cleanup: bench helpers created during investigation
drop function if exists __bench_duplicate(uuid);
drop function if exists __bench_duplicate_v2(uuid);
drop function if exists __bench_duplicate_v3(uuid);
